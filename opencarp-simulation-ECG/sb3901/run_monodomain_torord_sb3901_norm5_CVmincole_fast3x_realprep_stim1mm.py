...
def parser():
    ...
    parser.add_argument('--outdir',
                        default='./test_monodomain_torord_sb3901_norm5_CVminchole_fast3x_realprep_stim1mm',
...
def run(args, job):
    ...
    for i, (node, t) in enumerate(zip(nodes, times)):
        nearby_nodes = get_nodes_by_radius(PTS_PATH,node, radius=1000)
        ...
        stim_cmds += [
            ...
            f'-stim[{i}].pulse.strength', 53,  # 50uA/cm2 convertito da paper camps 53 pA/pF x 100 pF/cm2
    ...
    cmd += [
     ...
     '-prepacing_lats', os.path.join(args.inputdir, 'sb3901_meshes', 'init_acts_ACT_minchole3x-thresh.dat'),
    ]
# Aggiungi queste righe per chiedere a openCARP di calcolare i LAT
    cmd += [
         '-num_LATs', 1,
         '-lats[0].ID', 'ACT_minchole',
    ...
    cmd += [
        ...
        '-gregion[1].num_IDs', 1,
        '-gregion[1].ID[0]', 4,
        '-gregion[1].g_il', 0.2993 * 3.0,
        '-gregion[1].g_it', 0.1114 * 3.0, 
        '-gregion[1].g_in', 0.0601 * 3.0,
    ...
    cmd += [
        ...
        '-imp_region[1].ID[0]', 2,      #tag EPI invece di MID
        ...
        #'-adjustment[0].file', './sb3901_meshes/gks_tomek_gradient10percent.adj',
        #'-adjustment[0].dump', 1,
    ]
