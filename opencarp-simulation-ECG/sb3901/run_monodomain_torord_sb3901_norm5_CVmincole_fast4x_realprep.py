...
def parser():
    ...
    parser.add_argument('--outdir',
                        default='./test_monodomain_torord_sb3901_norm5_CVminchole_fast4x_prep0',
...
def run(args, job):
    ...
    cmd += [
     ...
     '-prepacing_lats', os.path.join(args.inputdir, 'sb3901_meshes', 'init_acts_ACT_minchole-thresh.dat'),
    ]
# Aggiungi queste righe per chiedere a openCARP di calcolare i LAT
    ...
    cmd += [
        ...
        '-gregion[0].ID[1]', 2,
        '-gregion[0].ID[2]', 3,
        '-gregion[0].g_il', 0.2993, #unità misura S/m calibrati con tunecv per CV 0.65 m/s
        '-gregion[0].g_it', 0.1114, # calibrata  per CV 0.36 m/s
        '-gregion[0].g_in', 0.0601, # calibrata  per CV 0.48 m/s
