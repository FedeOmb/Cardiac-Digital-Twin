...
def parser():
    ...
    parser.add_argument('--meshname',
                        default='./sb4101_meshes/sb4101_500um_tagged_fastendo',
                        help='Percorso base della mesh per openCARP')
    parser.add_argument('--vtx-file',
                        default='./sb4101_rootnodes/sb4101_fine500um_candidate_root_nodes.vtx',
                        help='File contenente gli indici dei nodi root (candidate_root_nodes.vtx)')
    parser.add_argument('--times-file',
                        default='./sb4101_rootnodes/sb4101_candidate_root_nodes_times.csv',
    ...
    parser.add_argument('--outdir',
                        default='./test_mono_torord_sb4101_norm5_CVminchole_fast4x_prep0',
...
def run(args, job):
    ...
    # ==========================================
    vtx_dir = os.path.join(args.outdir, 'sb4101_rootnodes_vtx')
    ...
    cmd += [
       ...
       # '-pstrat', 1,
        '-output_level', 5,
        '-phie_rec_ptf', os.path.join(args.inputdir, 'sb4101_meshes', 'sb4101_electrodes_um'),
        '-phie_recovery_file', 'sb4101_phie_recovery_norm2_prep_fast',
    ]
    ...
    cmd += [
        ...
        '-prepacing_lats', os.path.join(args.inputdir, 'sb4101_meshes', 'sb4101_latmap_zero.dat'),
    ]
# Aggiungi queste righe per chiedere a openCARP di calcolare i LAT
    ...
    cmd += [
        ...
        '-gregion[0].ID[1]', 2,
        '-gregion[0].ID[2]', 3,
        '-gregion[0].g_il', 0.2993, #unità misura S/m calibrati con tunecv per CV 0.57 m/s
        '-gregion[0].g_it', 0.1114, # calibrata  per CV 0.26 m/s
        '-gregion[0].g_in', 0.0601, # calibrata  per CV 0.16 m/s

        '-gregion[1].name', 'FastEndo',
        '-gregion[1].num_IDs', 1,
        '-gregion[1].ID[0]', 4,
        '-gregion[0].g_il', 0.2993 * 4.0,
        '-gregion[0].g_it', 0.1114 * 4.0,
