...
Tomek_editv2IonType::Tomek_editv2IonType(bool plugin) : IonType(std::move(std::string("Tomek_editv2")), plugin) {}

size_t Tomek_editv2IonType::params_size() const {
  return sizeof(struct Tomek_editv2_Params);
}

size_t Tomek_editv2IonType::dlo_vector_size() const {

  return 1;
}

uint32_t Tomek_editv2IonType::reqdat() const {
  return Tomek_editv2_REQDAT;
}

uint32_t Tomek_editv2IonType::moddat() const {
  return Tomek_editv2_MODDAT;
}

void Tomek_editv2IonType::destroy(IonIfBase& imp_base) const {
  IonIfDerived& imp = static_cast<IonIfDerived&>(imp_base);
  imp.destroy_luts();
  // rarely need to do anything else
}

Target Tomek_editv2IonType::select_target(Target target) const {
  switch (target) {
    case Target::AUTO:
#   ifdef TOMEK_EDITV2_MLIR_CUDA_GENERATED
      return Target::MLIR_CUDA;
#   elif defined(TOMEK_EDITV2_MLIR_ROCM_GENERATED)
      return Target::MLIR_ROCM;
#   elif defined(TOMEK_EDITV2_MLIR_CPU_GENERATED)
      return Target::MLIR_CPU;
#   elif defined(TOMEK_EDITV2_CPU_GENERATED)
      return Target::CPU;
#   else
...
void Tomek_editv2IonType::compute(Target target, int start, int end, IonIfBase& imp_base, GlobalData_t** data) const {
  IonIfDerived& imp = static_cast<IonIfDerived&>(imp_base);
  switch(target) {
    case Target::AUTO:
#   ifdef TOMEK_EDITV2_MLIR_CUDA_GENERATED
      compute_Tomek_editv2_mlir_gpu_cuda(start, end, imp, data);
#   elif defined(TOMEK_EDITV2_MLIR_ROCM_GENERATED)
      compute_Tomek_editv2_mlir_gpu_rocm(start, end, imp, data);
#   elif defined(TOMEK_EDITV2_MLIR_CPU_GENERATED)
      compute_Tomek_editv2_mlir_cpu(start, end, imp, data);
#   elif defined(TOMEK_EDITV2_CPU_GENERATED)
      compute_Tomek_editv2_cpu(start, end, imp, data);
#   else
#     error "Could not generate method Tomek_editv2IonType::compute."
#   endif
      break;
#   ifdef TOMEK_EDITV2_MLIR_CUDA_GENERATED
    case Target::MLIR_CUDA:
...
void Tomek_editv2IonType::initialize_params(IonIfBase& imp_base) const
{
  IonIfDerived& imp = static_cast<IonIfDerived&>(imp_base);
  cell_geom* region = &imp.cgeom();
  Tomek_editv2_Params *p = imp.params();
...
void Tomek_editv2IonType::construct_tables(IonIfBase& imp_base) const
{
  IonIfDerived& imp = static_cast<IonIfDerived&>(imp_base);
  ...
  LUT_alloc(Cai_tab, NROWS_Cai, 1e-6, 1e-2, 1e-6, "Tomek_editv2 Cai", imp.get_target());
  ...
  LUT_alloc(V_tab, NROWS_V, -1000, 1000, 1e-2, "Tomek_editv2 V", imp.get_target());
...
void Tomek_editv2IonType::initialize_sv(IonIfBase& imp_base, GlobalData_t **impdata ) const
{
  IonIfDerived& imp = static_cast<IonIfDerived&>(imp_base);
  GlobalData_t dt = imp.get_dt() * 1e0;
  cell_geom *region = &imp.cgeom();
  ...
  Tomek_editv2_state *sv_base = (Tomek_editv2_state *)imp.sv_tab().data();
  GlobalData_t t = 0;
  ...
  for(int __i=0; __i < imp.get_num_node(); __i+=1 ){
    Tomek_editv2_state *sv = sv_base+__i / 1;
    ...
    sv->Cansr = Cansr_init;
    sv->Cass = Cass_init;
    double GKs = ((p->celltype==1.) ? (sv->GKs_b*1.4) : sv->GKs_b);
    sv->I = I_init;
...
#ifdef TOMEK_EDITV2_CPU_GENERATED
extern "C" {
void compute_Tomek_editv2_cpu(int start, int end, IonIfBase& imp_base, GlobalData_t **impdata )
{
  Tomek_editv2IonType::IonIfDerived& imp = static_cast<Tomek_editv2IonType::IonIfDerived&>(imp_base);
  GlobalData_t dt = imp.get_dt()*1e0;
  cell_geom *region = &imp.cgeom();
  Tomek_editv2_Params *p  = imp.params();
  Tomek_editv2_state *sv_base = (Tomek_editv2_state *)imp.sv_tab().data();
  ...
  Tomek_editv2IonType::IonIfDerived* IF = &imp;
...
#pragma omp parallel for schedule(static)
  for (int __i=(start / 1) * 1; __i<end; __i+=1) {
    ...
    GlobalData_t GKs = ((p->celltype==1.) ? (sv->GKs_b*1.4) : sv->GKs_b);
...
#endif // TOMEK_EDITV2_CPU_GENERATED

bool Tomek_editv2IonType::has_trace() const {
    return true;
}

void Tomek_editv2IonType::trace(IonIfBase& imp_base, int node, FILE* file, GlobalData_t** impdata) const
{
  IonIfDerived& imp = static_cast<IonIfDerived&>(imp_base);
  static bool first = true;
  if (first) {
    first = false;
    FILE_SPEC theader = f_open("Tomek_editv2_trace_header.txt","wt");
    fprintf(theader->fd,
        "Iion\n"
        "V\n"
      );
  ...
  Tomek_editv2_state *sv = sv_base+node;
  int __i = node;
...
IonIfBase* Tomek_editv2IonType::make_ion_if(Target target, int num_node, const std::vector<std::reference_wrapper<IonType>>& plugins) const {
...
void Tomek_editv2IonType::destroy_ion_if(IonIfBase *imp) const {
