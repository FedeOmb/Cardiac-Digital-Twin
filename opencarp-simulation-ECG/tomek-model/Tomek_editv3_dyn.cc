...
void Tomek_editv3IonType::tune(IonIfBase& iif_base, const char* im_par) const {
  IonIfDerived& iif = static_cast<IonIfDerived&>(iif_base);
  Tomek_editv3_Params *p;							// pointer to parameter structure
  ...
  while( par ) {
    if( !strncmp(par, "flags=", 6 ) ) {
      ...
      bool valid_flags = verify_flags(Tomek_editv3_flags, par+6);
      if( !valid_flags ) {
        log_msg(_nc_logf, 5, 0, "Illegal flag specified: %s\n", par );
        exit( 1 );
      }
      ((Tomek_editv3_Params *)p)->flags = dupstr(par+6);
  ...
  while( par ) {
    ...
    if (0) ;
      ...
      CHANGE_PARAM( Tomek_editv3, p, CaMKo, mod );

    else if( !strcmp( "Cajsr_half", parameter ) )

      CHANGE_PARAM( Tomek_editv3, p, Cajsr_half, mod );

    else if( !strcmp( "GK1_b", parameter ) )

      CHANGE_PARAM( Tomek_editv3, p, GK1_b, mod );

    else if( !strcmp( "GKr_b", parameter ) )

      CHANGE_PARAM( Tomek_editv3, p, GKr_b, mod );

    else if( !strcmp( "GKs_b", parameter ) )

      CHANGE_PARAM( Tomek_editv3, p, GKs_b, mod );

    else if( !strcmp( "GNaL_b", parameter ) )

      CHANGE_PARAM( Tomek_editv3, p, GNaL_b, mod );

    else if( !strcmp( "Gncx_b", parameter ) )

      CHANGE_PARAM( Tomek_editv3, p, Gncx_b, mod );

    else if( !strcmp( "Gto_b", parameter ) )

      CHANGE_PARAM( Tomek_editv3, p, Gto_b, mod );

    else if( !strcmp( "Jrel_b", parameter ) )

      CHANGE_PARAM( Tomek_editv3, p, Jrel_b, mod );

    else if( !strcmp( "Jup_b", parameter ) )

      CHANGE_PARAM( Tomek_editv3, p, Jup_b, mod );

    else if( !strcmp( "PNaK_b", parameter ) )

      CHANGE_PARAM( Tomek_editv3, p, PNaK_b, mod );

    else if( !strcmp( "thL", parameter ) )

      CHANGE_PARAM( Tomek_editv3, p, thL, mod );

    else if( !strcmp( "tjCa", parameter ) )

      CHANGE_PARAM( Tomek_editv3, p, tjCa, mod );
 ...
 */
void Tomek_editv3IonType::print_params() const {
  IonIfDerived IF{*this, Target::AUTO, 0, {}};
  IF.initialize_params();
  printf("Name: Tomek_editv3\n" );
  printf("\tParameters:\n" );
...
int Tomek_editv3IonType::write_svs(IonIfBase& IF_base, FILE *out, int node) const {
  IonIfDerived& IF = static_cast<IonIfDerived&>(IF_base);
  fprintf( out, "%s\n", IF.get_type().get_name().c_str() );

  int inner_id = node % this->dlo_vector_size();

  Tomek_editv3_state *sv = IF.sv_tab().data()+(node / (this->dlo_vector_size()));
...
int Tomek_editv3IonType::read_svs(IonIfBase& IF_base, FILE *in) const {
  IonIfDerived& IF = static_cast<IonIfDerived&>(IF_base);
  ...
  Tomek_editv3_state *sv = IF.sv_tab().data();
...
SVgetfcn Tomek_editv3IonType::get_sv_offset(const char *svname, int *off, int *sz) const {
  SVgetfcn retall = (SVgetfcn)(1);


        Tomek_editv3_state *sv;

        if( !strcmp(svname,"ALL_SV") )  {

          *off  = 0;

          *sz   = sizeof(Tomek_editv3_state);
        ...
        if( !strcmp(svname,"C1") )  {
          ...
          *off  = offsetof(Tomek_editv3_state,C1);
        ...
        if( !strcmp(svname,"C2") )  {
          ...
          *off  = offsetof(Tomek_editv3_state,C2);
        ...
        if( !strcmp(svname,"C3") )  {
          ...
          *off  = offsetof(Tomek_editv3_state,C3);
        ...
        if( !strcmp(svname,"CaMKt") )  {
          ...
          *off  = offsetof(Tomek_editv3_state,CaMKt);
        ...
        if( !strcmp(svname,"Cai") )  {
          ...
          *off  = offsetof(Tomek_editv3_state,Cai);
        ...
        if( !strcmp(svname,"Cajsr") )  {
          ...
          *off  = offsetof(Tomek_editv3_state,Cajsr);
        ...
        if( !strcmp(svname,"Cansr") )  {
          ...
          *off  = offsetof(Tomek_editv3_state,Cansr);
        ...
        if( !strcmp(svname,"Cass") )  {
          ...
          *off  = offsetof(Tomek_editv3_state,Cass);
        ...
        if( !strcmp(svname,"GKs_b") )  {
          ...
          *off  = offsetof(Tomek_editv3_state,GKs_b);
        ...
        if( !strcmp(svname,"I") )  {
          ...
          *off  = offsetof(Tomek_editv3_state,I);
        ...
        if( !strcmp(svname,"Jrel_np") )  {
          ...
          *off  = offsetof(Tomek_editv3_state,Jrel_np);
        ...
        if( !strcmp(svname,"Jrel_p") )  {
          ...
          *off  = offsetof(Tomek_editv3_state,Jrel_p);
        ...
        if( !strcmp(svname,"Ki") )  {
          ...
          *off  = offsetof(Tomek_editv3_state,Ki);
        ...
        if( !strcmp(svname,"Kss") )  {
          ...
          *off  = offsetof(Tomek_editv3_state,Kss);
        ...
        if( !strcmp(svname,"Nai") )  {
          ...
          *off  = offsetof(Tomek_editv3_state,Nai);
        ...
        if( !strcmp(svname,"Nass") )  {
          ...
          *off  = offsetof(Tomek_editv3_state,Nass);
        ...
        if( !strcmp(svname,"O") )  {
          ...
          *off  = offsetof(Tomek_editv3_state,O);
        ...
        if( !strcmp(svname,"a") )  {
          ...
          *off  = offsetof(Tomek_editv3_state,a);
        ...
        if( !strcmp(svname,"ap") )  {
          ...
          *off  = offsetof(Tomek_editv3_state,ap);
        ...
        if( !strcmp(svname,"d") )  {
          ...
          *off  = offsetof(Tomek_editv3_state,d);
        ...
        if( !strcmp(svname,"fCaf") )  {
          ...
          *off  = offsetof(Tomek_editv3_state,fCaf);
        ...
        if( !strcmp(svname,"fCafp") )  {
          ...
          *off  = offsetof(Tomek_editv3_state,fCafp);
        ...
        if( !strcmp(svname,"fCas") )  {
          ...
          *off  = offsetof(Tomek_editv3_state,fCas);
        ...
        if( !strcmp(svname,"ff") )  {
          ...
          *off  = offsetof(Tomek_editv3_state,ff);
        ...
        if( !strcmp(svname,"ffp") )  {
          ...
          *off  = offsetof(Tomek_editv3_state,ffp);
        ...
        if( !strcmp(svname,"fs") )  {
          ...
          *off  = offsetof(Tomek_editv3_state,fs);
        ...
        if( !strcmp(svname,"h") )  {
          ...
          *off  = offsetof(Tomek_editv3_state,h);
        ...
        if( !strcmp(svname,"hL") )  {
          ...
          *off  = offsetof(Tomek_editv3_state,hL);
        ...
        if( !strcmp(svname,"hLp") )  {
          ...
          *off  = offsetof(Tomek_editv3_state,hLp);
        ...
        if( !strcmp(svname,"hp") )  {
          ...
          *off  = offsetof(Tomek_editv3_state,hp);
        ...
        if( !strcmp(svname,"iF") )  {
          ...
          *off  = offsetof(Tomek_editv3_state,iF);
        ...
        if( !strcmp(svname,"iFp") )  {
          ...
          *off  = offsetof(Tomek_editv3_state,iFp);
        ...
        if( !strcmp(svname,"iS") )  {
          ...
          *off  = offsetof(Tomek_editv3_state,iS);
        ...
        if( !strcmp(svname,"iSp") )  {
          ...
          *off  = offsetof(Tomek_editv3_state,iSp);
        ...
        if( !strcmp(svname,"j") )  {
          ...
          *off  = offsetof(Tomek_editv3_state,j);
        ...
        if( !strcmp(svname,"jCa") )  {
          ...
          *off  = offsetof(Tomek_editv3_state,jCa);
        ...
        if( !strcmp(svname,"jp") )  {
          ...
          *off  = offsetof(Tomek_editv3_state,jp);
        ...
        if( !strcmp(svname,"m") )  {
          ...
          *off  = offsetof(Tomek_editv3_state,m);
        ...
        if( !strcmp(svname,"mL") )  {
          ...
          *off  = offsetof(Tomek_editv3_state,mL);
        ...
        if( !strcmp(svname,"nCa_i") )  {
          ...
          *off  = offsetof(Tomek_editv3_state,nCa_i);
        ...
        if( !strcmp(svname,"nCa_ss") )  {
          ...
          *off  = offsetof(Tomek_editv3_state,nCa_ss);
        ...
        if( !strcmp(svname,"xs1") )  {
          ...
          *off  = offsetof(Tomek_editv3_state,xs1);
        ...
        if( !strcmp(svname,"xs2") )  {
          ...
          *off  = offsetof(Tomek_editv3_state,xs2);
...
int Tomek_editv3IonType::get_sv_list(char*** list) const {
...
#define BOGUSTYPE -1
int Tomek_editv3IonType::get_sv_type(const char *svname, int *type, char **Typename) const
{
  *type = BOGUSTYPE;
  if (0) ;
...
void Tomek_editv3IonType::print_metadata() const {
  printf("Metadata:\n");
...
extern "C" {
    limpet::Tomek_editv3IonType* __new_IonType(bool plugin) {
              return new limpet::Tomek_editv3IonType(plugin);
    }
}
