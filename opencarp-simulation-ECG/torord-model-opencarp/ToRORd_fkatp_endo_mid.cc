...
#include "ToRORd_fkatp_endo_mid.h"
...
ToRORd_fkatp_endo_midIonType::ToRORd_fkatp_endo_midIonType(bool plugin) : IonType(std::move(std::string("ToRORd_fkatp_endo_mid")), plugin) {}

size_t ToRORd_fkatp_endo_midIonType::params_size() const {
  return sizeof(struct ToRORd_fkatp_endo_mid_Params);
}

size_t ToRORd_fkatp_endo_midIonType::dlo_vector_size() const {

  return 1;
}

uint32_t ToRORd_fkatp_endo_midIonType::reqdat() const {
  return ToRORd_fkatp_endo_mid_REQDAT;
}

uint32_t ToRORd_fkatp_endo_midIonType::moddat() const {
  return ToRORd_fkatp_endo_mid_MODDAT;
}

void ToRORd_fkatp_endo_midIonType::destroy(IonIfBase& imp_base) const {
  IonIfDerived& imp = static_cast<IonIfDerived&>(imp_base);
  imp.destroy_luts();
  // rarely need to do anything else
}

Target ToRORd_fkatp_endo_midIonType::select_target(Target target) const {
  switch (target) {
    case Target::AUTO:
#   ifdef TORORD_FKATP_ENDO_MID_MLIR_CUDA_GENERATED
      return Target::MLIR_CUDA;
#   elif defined(TORORD_FKATP_ENDO_MID_MLIR_ROCM_GENERATED)
      return Target::MLIR_ROCM;
#   elif defined(TORORD_FKATP_ENDO_MID_MLIR_CPU_GENERATED)
      return Target::MLIR_CPU;
#   elif defined(TORORD_FKATP_ENDO_MID_CPU_GENERATED)
      return Target::CPU;
#   else
...
void ToRORd_fkatp_endo_midIonType::compute(Target target, int start, int end, IonIfBase& imp_base, GlobalData_t** data) const {
  IonIfDerived& imp = static_cast<IonIfDerived&>(imp_base);
  switch(target) {
    case Target::AUTO:
#   ifdef TORORD_FKATP_ENDO_MID_MLIR_CUDA_GENERATED
      compute_ToRORd_fkatp_endo_mid_mlir_gpu_cuda(start, end, imp, data);
#   elif defined(TORORD_FKATP_ENDO_MID_MLIR_ROCM_GENERATED)
      compute_ToRORd_fkatp_endo_mid_mlir_gpu_rocm(start, end, imp, data);
#   elif defined(TORORD_FKATP_ENDO_MID_MLIR_CPU_GENERATED)
      compute_ToRORd_fkatp_endo_mid_mlir_cpu(start, end, imp, data);
#   elif defined(TORORD_FKATP_ENDO_MID_CPU_GENERATED)
      compute_ToRORd_fkatp_endo_mid_cpu(start, end, imp, data);
#   else
#     error "Could not generate method ToRORd_fkatp_endo_midIonType::compute."
#   endif
      break;
#   ifdef TORORD_FKATP_ENDO_MID_MLIR_CUDA_GENERATED
    case Target::MLIR_CUDA:
...
#define C1_init (GlobalData_t)(0.00069560)
#define C2_init (GlobalData_t)(0.00082672)
#define C3_init (GlobalData_t)(0.9979)
#define CaMKo (GlobalData_t)(0.05)
#define CaMKt_init (GlobalData_t)(0.0192)
...
#define I_init (GlobalData_t)(1.8784e-05)
#define Jrel_b (GlobalData_t)(1.5378)
#define Jrel_np_init (GlobalData_t)(0.)
...
#define O_init (GlobalData_t)(0.00054206)
...
#define a_init (GlobalData_t)(0.00092527)
#define alpha_1 (GlobalData_t)(0.154375)
#define ap_init (GlobalData_t)(0.00047143)
#define bCaMK (GlobalData_t)(0.00068)
#define beta_1 (GlobalData_t)(0.1911)
#define bt (GlobalData_t)(4.75)
#define cai_init (GlobalData_t)(8.166e-05)
#define cajsr_half (GlobalData_t)(1.7)
#define cajsr_init (GlobalData_t)(1.9593)
#define cansr_init (GlobalData_t)(1.9557)
#define cao (GlobalData_t)(1.8)
#define cass_init (GlobalData_t)(6.5781e-05)
#define celltype (GlobalData_t)(2.)
...
#define fcas_init (GlobalData_t)(0.9998)
#define ff_init (GlobalData_t)(1.0)
#define ffp_init (GlobalData_t)(1.0)
#define fkatp (GlobalData_t)(0.0)
#define fs_init (GlobalData_t)(0.92)
#define gkatp (GlobalData_t)(4.3195)
#define hL_init (GlobalData_t)(0.5327)
#define hLp_init (GlobalData_t)(0.2834)
#define h_init (GlobalData_t)(0.8365)
#define hp_init (GlobalData_t)(0.6838)
#define iF_init (GlobalData_t)(0.9996)
#define iFp_init (GlobalData_t)(0.9996)
#define iS_init (GlobalData_t)(0.5671)
#define iSp_init (GlobalData_t)(0.6261)
...
#define j_init (GlobalData_t)(0.8363)
#define jca_init (GlobalData_t)(1.0)
#define jp_init (GlobalData_t)(0.8358)
...
#define ki_init (GlobalData_t)(143.0403)
...
#define kss_init (GlobalData_t)(143.0402)
#define mL_init (GlobalData_t)(0.00015079)
#define m_init (GlobalData_t)(0.00073818)
#define nai_init (GlobalData_t)(15.0038)
#define nao (GlobalData_t)(140.0)
#define nass_init (GlobalData_t)(15.0043)
#define nca_i_init (GlobalData_t)(0.0012)
#define nca_ss_init (GlobalData_t)(0.00051399)
...
#define v_init (GlobalData_t)(-89.1704)
#define wca (GlobalData_t)(6e4)
#define wna (GlobalData_t)(6e4)
#define wnaca (GlobalData_t)(5e3)
#define xs1_init (GlobalData_t)(0.2653)
#define xs2_init (GlobalData_t)(0.00016921)
...
void ToRORd_fkatp_endo_midIonType::initialize_params(IonIfBase& imp_base) const
{
  IonIfDerived& imp = static_cast<IonIfDerived&>(imp_base);
  cell_geom* region = &imp.cgeom();
  ToRORd_fkatp_endo_mid_Params *p = imp.params();

  // Compute the regional constants
  {

#ifndef USE_CVODE
        //log_msg(NULL,5,0,"ToRORd_fkatp_endo_mid needs CVODE. Recompilation needed.");
        exit(1);
#endif
  }
...
void ToRORd_fkatp_endo_midIonType::construct_tables(IonIfBase& imp_base) const
{
  IonIfDerived& imp = static_cast<IonIfDerived&>(imp_base);
...
int ToRORd_fkatp_endo_mid_internal_rhs_function(sunrealtype t, N_Vector CVODE, N_Vector CVODE_dot, void* user_data);
#else
int ToRORd_fkatp_endo_mid_internal_rhs_function(realtype t, N_Vector CVODE, N_Vector CVODE_dot, void* user_data);
#endif
#endif
...
void ToRORd_fkatp_endo_midIonType::initialize_sv(IonIfBase& imp_base, GlobalData_t **impdata ) const
{
  IonIfDerived& imp = static_cast<IonIfDerived&>(imp_base);
  GlobalData_t dt = imp.get_dt() * 1e0;
  cell_geom *region = &imp.cgeom();
  ...
  ToRORd_fkatp_endo_mid_state *sv_base = (ToRORd_fkatp_endo_mid_state *)imp.sv_tab().data();
  GlobalData_t t = 0;
  ...
  for(int __i=0; __i < imp.get_num_node(); __i+=1 ){
    ToRORd_fkatp_endo_mid_state *sv = sv_base+__i / 1;
  ...
  //setup CVODE
  {
...
#endif
    int flag = CVodeInit(cvode_mem, ToRORd_fkatp_endo_mid_internal_rhs_function, 0, cvode_y);
    ...
    assert(flag == CV_SUCCESS);
    ToRORd_fkatp_endo_mid_Private_cpu* userdata = (ToRORd_fkatp_endo_mid_Private_cpu*)(imp.ion_private().data());
...
#ifdef TORORD_FKATP_ENDO_MID_CPU_GENERATED
extern "C" {
void compute_ToRORd_fkatp_endo_mid_cpu(int start, int end, IonIfBase& imp_base, GlobalData_t **impdata )
{
  ToRORd_fkatp_endo_midIonType::IonIfDerived& imp = static_cast<ToRORd_fkatp_endo_midIonType::IonIfDerived&>(imp_base);
  GlobalData_t dt = imp.get_dt()*1e0;
  cell_geom *region = &imp.cgeom();
  ToRORd_fkatp_endo_mid_Params *p  = imp.params();
  ToRORd_fkatp_endo_mid_state *sv_base = (ToRORd_fkatp_endo_mid_state *)imp.sv_tab().data();
  ...
  ToRORd_fkatp_endo_midIonType::IonIfDerived* IF = &imp;

  // Define the constants that depend on the parameters.
  
  //Initializing the userdata structures.
  ToRORd_fkatp_endo_midIonType::private_type* ion_private = imp.ion_private().data();
  int nthread = 1;
  #ifdef _OPENMP
  ...
  N_Vector CVODE = N_VMake_Serial(N_CVODE, CVODE_array, *((SUNContext*)((ToRORd_fkatp_endo_midIonType::private_type*)(imp.ion_private().data()))->sunctx_ptr));
  #else
  realtype CVODE_array[N_CVODE];
  N_Vector CVODE = N_VMake_Serial(N_CVODE, CVODE_array);
  #endif
  #endif
  void* cvode_mem = ((ToRORd_fkatp_endo_midIonType::private_type*)(imp.ion_private().data()))->cvode_mem;
...
#endif // TORORD_FKATP_ENDO_MID_CPU_GENERATED
...
int ToRORd_fkatp_endo_mid_internal_rhs_function(realtype t, N_Vector CVODE, N_Vector CVODE_dot, void* user_data) {
  ...
  ToRORd_fkatp_endo_midIonType::private_type* ion_private = (ToRORd_fkatp_endo_midIonType::private_type*)user_data;
  ToRORd_fkatp_endo_mid_Private_cpu::regional_constants_type* rc = &ion_private->rc;
  ToRORd_fkatp_endo_midIonType::IonIfDerived *IF = static_cast<ToRORd_fkatp_endo_midIonType::IonIfDerived*>(ion_private->IF);
  ToRORd_fkatp_endo_mid_Nodal_Req_cpu* nr = &ion_private->nr;
  int __i = ((ToRORd_fkatp_endo_midIonType::private_type*)user_data)->node_number;
  cell_geom *region = &IF->cgeom();
  ToRORd_fkatp_endo_mid_Params *p = IF->params();
  ToRORd_fkatp_endo_mid_state *sv_base = IF->sv_tab().data();
  ToRORd_fkatp_endo_mid_state *sv = sv_base+__i;
  int inner_id = 0;
...
bool ToRORd_fkatp_endo_midIonType::has_trace() const {
    return false;
}

void ToRORd_fkatp_endo_midIonType::trace(IonIfBase& imp_base, int node, FILE* file, GlobalData_t** data) const {}
IonIfBase* ToRORd_fkatp_endo_midIonType::make_ion_if(Target target, int num_node, const std::vector<std::reference_wrapper<IonType>>& plugins) const {
...
void ToRORd_fkatp_endo_midIonType::destroy_ion_if(IonIfBase *imp) const {
