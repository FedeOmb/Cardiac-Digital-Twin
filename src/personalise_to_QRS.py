...
def run_qrs_personalization(anatomy_subject_name, ecg_subject_name, **kwargs):
    ...
    # ####################################################################################################################
    print('Caution, all the hyper-parameters are set assuming resolutions of 1000 Hz in all time-series.')  # TODO: make hyper-parameters relative to the time series resolutions.
...
get_purkinje_speed_name, get_xyz_name_list, unfold_ecg_matrix
    ...
    if not os.path.exists(results_dir_part):
        os.mkdir(results_dir_part)
    # Use date to name the result folder to preserve some history of result
    ...
    approx_djikstra_purkinje_max_path_len = kwargs.get('approx_djikstra_purkinje_max_path_len', 200)
    lv_inter_root_node_distance = kwargs.get('lv_inter_root_node_distance', 1.5)  # 1.5 cm    # TODO: Calibrate this hyper-parameter using sensitivity analysis
    rv_inter_root_node_distance = kwargs.get('rv_inter_root_node_distance', 1.5)  # 1.5 cm    # TODO: Calibrate this hyper-parameter using sensitivity analysis
    ...
    max_len_qrs = kwargs.get('max_len_qrs', 200)  # This hyper-paramter is used when paralelising the ecg computation, because it needs a structure to synchronise the results from the multiple threads.
    ...
    parameter_fixed_value_dict[fibre_speed_name] = kwargs.get('fibre_speed', 0.065)  # Taggart et al. (2000)
    parameter_fixed_value_dict[normal_speed_name] = kwargs.get('normal_speed', 0.048)  # Taggart et al. (2000)
    parameter_fixed_value_dict[purkinje_speed_name] = kwargs.get('purkinje_speed', 0.3)  # Made up value (cm/ms)
    ...
    # Speeds
    sheet_speed_range = kwargs.get('sheet_speed_range', [0.025, 0.06])  # cm/ms
    sheet_speed_prior = kwargs.get('sheet_speed_prior', None)  # [mean, std]
    dense_endo_speed_range = kwargs.get('dense_endo_speed_range', [0.1, 0.19])  # cm/ms
    dense_endo_speed_prior = kwargs.get('dense_endo_speed_prior', None)  # [mean, std]
    sparse_endo_speed_range = kwargs.get('sparse_endo_speed_range', [0.07, 0.15])  # cm/ms
    sparse_endo_speed_prior = kwargs.get('sparse_endo_speed_prior', None)  # [mean, std]
    # Root Nodes
    nb_root_nodes_range = kwargs.get('nb_root_nodes_range', [3, 9])  # min/max -> 10 is computationally intractable
    nb_root_node_prior = kwargs.get('nb_root_node_prior', [6, 1])  # [mean, std]
    ...
    population_size = kwargs.get('population_size', 120)  # 512   # Rule of thumb number    # TODO: Calibrate this hyper-parameter using sensitivity analysis
    ...
    desired_discrepancy = kwargs.get('desired_discrepancy', 1.0)  #1. #0.1 # This value needs to be changed with respect of what discrepancy metric you want to use.  # this value is for the DTW metric was 0.35  # After several tests was found good with the latest discrepancy metric strategy
    ...
    visualisation_count = kwargs.get('visualisation_count', 50)  # Minimum of 1 to avoid division by zero
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        ...
        ecg_subject_name = 'sb4301'  # Allows using a different ECG for the personalisation than for the anatomy
    else:
        anatomy_subject_name = sys.argv[1]
        ecg_subject_name = sys.argv[1]
    
    run_qrs_personalization(anatomy_subject_name=anatomy_subject_name, ecg_subject_name=ecg_subject_name)
