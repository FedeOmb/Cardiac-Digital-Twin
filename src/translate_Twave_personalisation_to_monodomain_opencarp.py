...
def translate_Twave_pers_to_opencarp(anatomy_subject_name, ecg_subject_name, **kwargs):
...
# if False:
    ...
    uncertainty_proportion = kwargs.get('uncertainty_proportion', 0.1)   # 10% of the population size
    ...
    with pymp.Parallel(min(threadsNum, uncertainty_parameter_population.shape[0])) as p1:
        for uncertainty_i in p1.range(uncertainty_parameter_population.shape[0]):
            ...
            uncertainty_biomarker_result_file_name = for_monodomain_biomarker_result_file_name_start + iteration_str_tag
            ...
            uncertainty_biomarker_result_file_name_csv = uncertainty_biomarker_result_file_name + '.csv'
            if not os.path.exists(uncertainty_biomarker_result_file_name_csv):
                ...
                save_pandas(df=node_biomarker, filename=uncertainty_biomarker_result_file_name_csv)
                p1.print('Saved: ', uncertainty_biomarker_result_file_name_csv)

                # --- OPENCARP EXPORT (.adj per i fattori di scala dei canali) ---
                for ionic_scaling_name in gradient_ion_channel_list:
                    uncertainty_biomarker_result_file_name_adj = uncertainty_biomarker_result_file_name + '.adj'
                    scaling_values = node_biomarker[ionic_scaling_name].values
                    with open(uncertainty_biomarker_result_file_name_adj, 'w') as f:
                        f.write(f"{len(scaling_values)}\nintra\n")
                        for idx_node, val in enumerate(scaling_values):
                            f.write(f"{idx_node} {val:.8f}\n")
                    p1.print(f'Saved openCARP .adj file for {ionic_scaling_name} at {uncertainty_biomarker_result_file_name_adj}')

                # --- OPENCARP EXPORT (.dat per i campi scalari di confronto) ---
                fields_to_export = [
                    (activation_time_map_biomarker_name, 'lat'),
                    (repolarisation_time_map_biomarker_name, 'repol'),
                    (apd90_biomarker_name, 'apd90')
                ]
                for field_col, field_suffix in fields_to_export:
                    dat_filename = uncertainty_biomarker_result_file_name + f'_{field_suffix}.dat'
                    np.savetxt(dat_filename, node_biomarker[field_col].values, fmt='%.8f')
                    p1.print(f'Saved openCARP .dat file for {field_suffix} at {dat_filename}')
                # Esporta ARI (Repol - LAT)
                ari_values = node_biomarker[repolarisation_time_map_biomarker_name].values - node_biomarker[activation_time_map_biomarker_name].values
                ari_filename = uncertainty_biomarker_result_file_name + '_ARI.dat'
                np.savetxt(ari_filename, ari_values, fmt='%.8f')
                p1.print(f'Saved openCARP .dat file for ARI at {ari_filename}')
...
if __name__ == '__main__':
    ...
    for anatomy_subject_name in anatomy_subject_name_list:
        ...
        translate_Twave_pers_to_opencarp(anatomy_subject_name=anatomy_subject_name, ecg_subject_name=ecg_subject_name)