...
ToRORd_fkatp_endoIonType::ToRORd_fkatp_endoIonType(bool plugin) : IonType(std::move(std::string("ToRORd_fkatp_endo")), plugin) {}

size_t ToRORd_fkatp_endoIonType::params_size() const {
  return sizeof(struct ToRORd_fkatp_endo_Params);
}

size_t ToRORd_fkatp_endoIonType::dlo_vector_size() const {

  return 1;
}

uint32_t ToRORd_fkatp_endoIonType::reqdat() const {
  return ToRORd_fkatp_endo_REQDAT;
}

uint32_t ToRORd_fkatp_endoIonType::moddat() const {
  return ToRORd_fkatp_endo_MODDAT;
}

void ToRORd_fkatp_endoIonType::destroy(IonIfBase& imp_base) const {
  IonIfDerived& imp = static_cast<IonIfDerived&>(imp_base);
  imp.destroy_luts();
  // rarely need to do anything else
}

Target ToRORd_fkatp_endoIonType::select_target(Target target) const {
  switch (target) {
    case Target::AUTO:
#   ifdef TORORD_FKATP_ENDO_MLIR_CUDA_GENERATED
      return Target::MLIR_CUDA;
#   elif defined(TORORD_FKATP_ENDO_MLIR_ROCM_GENERATED)
      return Target::MLIR_ROCM;
#   elif defined(TORORD_FKATP_ENDO_MLIR_CPU_GENERATED)
      return Target::MLIR_CPU;
#   elif defined(TORORD_FKATP_ENDO_CPU_GENERATED)
      return Target::CPU;
#   else
...
#   ifdef TORORD_FKATP_ENDO_MLIR_CUDA_GENERATED
    case Target::MLIR_CUDA:
      return Target::MLIR_CUDA;
#   endif
#   ifdef TORORD_FKATP_ENDO_MLIR_ROCM_GENERATED
    case Target::MLIR_ROCM:
      return Target::MLIR_ROCM;
#   endif
#   ifdef TORORD_FKATP_ENDO_MLIR_CPU_GENERATED
    case Target::MLIR_CPU:
      return Target::MLIR_CPU;
#   endif
#   ifdef TORORD_FKATP_ENDO_CPU_GENERATED
    case Target::CPU:
      return Target::CPU;
#   endif
    default:
...
void ToRORd_fkatp_endoIonType::compute(Target target, int start, int end, IonIfBase& imp_base, GlobalData_t** data) const {
  IonIfDerived& imp = static_cast<IonIfDerived&>(imp_base);
  switch(target) {
    case Target::AUTO:
#   ifdef TORORD_FKATP_ENDO_MLIR_CUDA_GENERATED
      compute_ToRORd_fkatp_endo_mlir_gpu_cuda(start, end, imp, data);
#   elif defined(TORORD_FKATP_ENDO_MLIR_ROCM_GENERATED)
      compute_ToRORd_fkatp_endo_mlir_gpu_rocm(start, end, imp, data);
#   elif defined(TORORD_FKATP_ENDO_MLIR_CPU_GENERATED)
      compute_ToRORd_fkatp_endo_mlir_cpu(start, end, imp, data);
#   elif defined(TORORD_FKATP_ENDO_CPU_GENERATED)
      compute_ToRORd_fkatp_endo_cpu(start, end, imp, data);
#   else
#     error "Could not generate method ToRORd_fkatp_endoIonType::compute."
#   endif
      break;
#   ifdef TORORD_FKATP_ENDO_MLIR_CUDA_GENERATED
    case Target::MLIR_CUDA:
...
#define celltype (GlobalData_t)(0.)
...
void ToRORd_fkatp_endoIonType::initialize_params(IonIfBase& imp_base) const
{
  IonIfDerived& imp = static_cast<IonIfDerived&>(imp_base);
  cell_geom* region = &imp.cgeom();
  ToRORd_fkatp_endo_Params *p = imp.params();

  // Compute the regional constants
  {

#ifndef USE_CVODE
        //log_msg(NULL,5,0,"ToRORd_fkatp_endo needs CVODE. Recompilation needed.");
        exit(1);
#endif
  }
...
void ToRORd_fkatp_endoIonType::construct_tables(IonIfBase& imp_base) const
{
  IonIfDerived& imp = static_cast<IonIfDerived&>(imp_base);
...
int ToRORd_fkatp_endo_internal_rhs_function(sunrealtype t, N_Vector CVODE, N_Vector CVODE_dot, void* user_data);
#else
int ToRORd_fkatp_endo_internal_rhs_function(realtype t, N_Vector CVODE, N_Vector CVODE_dot, void* user_data);
#endif
#endif
...
void ToRORd_fkatp_endoIonType::initialize_sv(IonIfBase& imp_base, GlobalData_t **impdata ) const
{
  IonIfDerived& imp = static_cast<IonIfDerived&>(imp_base);
  GlobalData_t dt = imp.get_dt() * 1e0;
  cell_geom *region = &imp.cgeom();
  ...
  ToRORd_fkatp_endo_state *sv_base = (ToRORd_fkatp_endo_state *)imp.sv_tab().data();
  GlobalData_t t = 0;
  ...
  for(int __i=0; __i < imp.get_num_node(); __i+=1 ){
    ToRORd_fkatp_endo_state *sv = sv_base+__i / 1;
  ...
  //setup CVODE
  {
...
#endif
    int flag = CVodeInit(cvode_mem, ToRORd_fkatp_endo_internal_rhs_function, 0, cvode_y);
    ...
    assert(flag == CV_SUCCESS);
    ToRORd_fkatp_endo_Private_cpu* userdata = (ToRORd_fkatp_endo_Private_cpu*)(imp.ion_private().data());
...
#ifdef TORORD_FKATP_ENDO_CPU_GENERATED
extern "C" {
void compute_ToRORd_fkatp_endo_cpu(int start, int end, IonIfBase& imp_base, GlobalData_t **impdata )
{
  ToRORd_fkatp_endoIonType::IonIfDerived& imp = static_cast<ToRORd_fkatp_endoIonType::IonIfDerived&>(imp_base);
  GlobalData_t dt = imp.get_dt()*1e0;
  cell_geom *region = &imp.cgeom();
  ToRORd_fkatp_endo_Params *p  = imp.params();
  ToRORd_fkatp_endo_state *sv_base = (ToRORd_fkatp_endo_state *)imp.sv_tab().data();
  ...
  ToRORd_fkatp_endoIonType::IonIfDerived* IF = &imp;

  // Define the constants that depend on the parameters.
  
  //Initializing the userdata structures.
  ToRORd_fkatp_endoIonType::private_type* ion_private = imp.ion_private().data();
  int nthread = 1;
  #ifdef _OPENMP
  ...
  N_Vector CVODE = N_VMake_Serial(N_CVODE, CVODE_array, *((SUNContext*)((ToRORd_fkatp_endoIonType::private_type*)(imp.ion_private().data()))->sunctx_ptr));
  #else
  realtype CVODE_array[N_CVODE];
  N_Vector CVODE = N_VMake_Serial(N_CVODE, CVODE_array);
  #endif
  #endif
  void* cvode_mem = ((ToRORd_fkatp_endoIonType::private_type*)(imp.ion_private().data()))->cvode_mem;
...
#endif // TORORD_FKATP_ENDO_CPU_GENERATED
...
int ToRORd_fkatp_endo_internal_rhs_function(realtype t, N_Vector CVODE, N_Vector CVODE_dot, void* user_data) {
  ...
  ToRORd_fkatp_endoIonType::private_type* ion_private = (ToRORd_fkatp_endoIonType::private_type*)user_data;
  ToRORd_fkatp_endo_Private_cpu::regional_constants_type* rc = &ion_private->rc;
  ToRORd_fkatp_endoIonType::IonIfDerived *IF = static_cast<ToRORd_fkatp_endoIonType::IonIfDerived*>(ion_private->IF);
  ToRORd_fkatp_endo_Nodal_Req_cpu* nr = &ion_private->nr;
  int __i = ((ToRORd_fkatp_endoIonType::private_type*)user_data)->node_number;
  cell_geom *region = &IF->cgeom();
  ToRORd_fkatp_endo_Params *p = IF->params();
  ToRORd_fkatp_endo_state *sv_base = IF->sv_tab().data();
  ToRORd_fkatp_endo_state *sv = sv_base+__i;
  int inner_id = 0;
...
bool ToRORd_fkatp_endoIonType::has_trace() const {
    return false;
}

void ToRORd_fkatp_endoIonType::trace(IonIfBase& imp_base, int node, FILE* file, GlobalData_t** data) const {}
IonIfBase* ToRORd_fkatp_endoIonType::make_ion_if(Target target, int num_node, const std::vector<std::reference_wrapper<IonType>>& plugins) const {
...
void ToRORd_fkatp_endoIonType::destroy_ion_if(IonIfBase *imp) const {
